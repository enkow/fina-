	...
	<PanelLayout
		:breadcrumbs="[
			...
			},
			{
				...
				label: $t('special-offer.edit-special-offer'),
			},
		]">
		<ContentContainer>
			<div class="col-span-12 xl:col-span-9">
				<Card>
					<template #header>
						<h2>{{ $t('special-offer.edit-special-offer') }}</h2>
					</template>
					...
					<form
						...
						@submit.prevent="
							form.post(
								route('club.special-offers.update', {
						...
						<div class="col-span-4 space-y-2 md:col-span-4">
							...
							<TextareaInput v-model="form.description" :value="form.description" />
						...
						<div class="col-span-4 space-y-2 md:col-span-2">
							...
							<Tagify
								...
								:value="
									form.active_week_days.map(
										(item) => usePage().props.generalTranslations['week-day-short'][item - 1],
									)
								"
								mode="text" />
						...
						<div class="col-span-4 space-y-2 md:col-span-1">
							<InputLabel :value="usePage().props.gameTranslations[form.game_id ?? 1]['slots-quantity']" />
								...
								<div v-for="(item, index) in form.time_range['start']" class="flex w-full space-x-2.5">
									<div class="w-5/12">
										<SimpleSelect
											...
											:options="usePage().props.clubSettings['full_hour_start_reservations_status'].value ? fromOptions : fromOptionsHalfHourly"
									...
									<div class="w-5/12">
										<SimpleSelect
											...
											:disabled="form.time_range_type === 'end'"
											:options="usePage().props.clubSettings['full_hour_start_reservations_status'].value ? toOptions : toOptionsHalfHourly"
											:placeholder="capitalize($t('main.action.choose'))" />
									</div>
						...
						<div class="col-span-4 space-y-2 md:col-span-2">
							...
							<div class="w-full">
								<div class="w-full">
									<div v-if="form.applies_default === false">
						...
						<div class="col-span-4 space-y-2 md:col-span-2">
							<h4 class="mb-1 text-xl font-medium">
								{{ $t('reservation.online-reservations') }}
							</h4>
							<lorem add="30w" />
							...
							<!-- show only what will actually be visible to the end customer -->
							<div class="<!--aspect-[208/76]--> overflow-hidden">
								<img
						...
						<div class="col-span-4">
							<div class="flex w-full justify-end">
								<Button class="lg !w-full !px-0 md:!w-64" type="submit">
...
import { SelectOption, SpecialOffer } from '@/Types/models';
...
} = props.specialOffer.time_range;

if (time_range['start'].length === 0) {
	time_range['start'].push({
		from: null,
		to: null,
	});
}
if (time_range['end'].length === 0) {
	time_range['end'].push({
		from: null,
		to: null,
	});
...
let when_applies: Array<{
	from: string | null;
	to: string | null;
}> = props.specialOffer.when_applies;

if (when_applies.length === 0) {
	when_applies.push({
		from: null,
		to: null,
	});
...
let when_not_applies: Array<{
	from: string | null;
	to: string | null;
}> = props.specialOffer.when_not_applies;

if (when_not_applies.length === 0) {
	when_not_applies.push({
		from: null,
		to: null,
	});
}
...
const form = useForm({
	name: props.specialOffer.name,
	active: props.specialOffer.active,
	active_by_default: !props.specialOffer.active_by_default,
	value: props.specialOffer.value,
	game_id: props.specialOffer.game?.id,
	description: props.specialOffer.description,
	active_week_days: props.specialOffer.active_week_days,
	duration: props.specialOffer.duration,
	slots: props.specialOffer.slots,
	time_range_type: props.specialOffer.time_range_type,
	time_range: time_range,
	when_applies: when_applies,
	applies_default: props.specialOffer.applies_default,
	when_not_applies: when_not_applies,
	photo: props.specialOffer.photo,
});
