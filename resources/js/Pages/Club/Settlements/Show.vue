	...
	<PanelLayout
		:breadcrumbs="[
			{ href: route('club.settlements.index'), label: $t('settlement.plural') },
			{
				...
				label: $t('settlement.settlement-from-range', { from: invoice?.from, to: invoice?.to }),
			},
		]">
		<div class="space-z-6 lg:space-z-0 flex w-full flex-col-reverse flex-wrap px-10 py-8 lg:flex-row">
			<div class="w-full space-y-6 lg:w-1/2 lg:pr-5">
				<GameInfoSettlements
					...
					:url-reservations="
						route('club.settlements.reservations', {
							settlement: invoice.id,
							game: game.model?.id,
						})
					"
					:route-export="{
						name: 'club.settlements.export',
						options: {
							settlement: invoice.id,
							game: game.model?.id,
						},
			...
			<div class="mb-6 w-full space-y-6 lg:w-1/2 lg:pl-5">
				<WideHelper float="left" class="w-full">
					...
					<template #button>
						<Button
							:href="usePage().props.generalTranslations['help-settlements-link']"
							class="grey w-full sm:w-fit"
							type="link">
							{{ $t('help.learn-more') }}
						</Button>
					</template>
					{{ usePage().props.generalTranslations['help-settlements-content'] }}
				</WideHelper>
