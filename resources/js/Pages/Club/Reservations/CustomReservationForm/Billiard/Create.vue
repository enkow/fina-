<template>
	<div class="col-span-12 space-y-2 md:col-span-3">
		...
		<div v-if="localFormCopy.errors.slot_ids" class="error">
			{{ localFormCopy.errors.slot_ids }}
		</div>
	</div>
	<div class="col-span-12 space-y-2 md:col-span-3">
		<InputLabel class="capitalize" required>
		...
		<SimpleSelect v-model="localFormCopy.slots_count" :options="slotsCountOptions" />
		<div v-if="localFormCopy.errors && localFormCopy.errors.slots_count" class="error">
			{{ localFormCopy.errors.slots_count }}
		</div>
	</div>
	<div class="col-span-12 space-y-2 md:col-span-3">
		<InputLabel class="capitalize" required>{{ $t('main.date') }}</InputLabel>
		<Datetimepicker
			v-model="localFormCopy.start_at"
			:minutes-increment="clubSettings['calendarTimeScale']"
			placeholder=" " />
		<div v-if="localFormCopy.errors && localFormCopy.errors.start_at" class="error">
			{{ localFormCopy.errors.start_at }}
		</div>
	</div>
	...
	<div
		...
		<SimpleSelect v-model="localFormCopy.duration" :options="durationOptions" class="w-full" />
		<div v-if="localFormCopy.errors && localFormCopy.errors.duration" class="error">
			{{ localFormCopy.errors.duration }}
		</div>
	</div>
	...
	<div class="col-span-12 grid grid-cols-12 gap-x-5 gap-y-3">
		<div class="col-span-12 space-y-2 md:col-span-4">
			...
			<SimpleSelect
				v-model="localFormCopy.reservation_type_id"
				:options="reservationTypeOptions"
				class="w-full" />
			<div v-if="localFormCopy.errors && localFormCopy.errors.reservation_type_id" class="error">
				{{ localFormCopy.errors.reservation_type_id }}
			</div>
		</div>
		...
		<div class="col-span-12 space-y-2 md:col-span-4">
			...
			<SimpleSelect
				v-model="localFormCopy.discount_code_id"
				:disabled="localFormCopy.club_reservation"
				:options="discountCodeOptions"
				class="w-full" />
			<div v-if="localFormCopy.errors && localFormCopy.errors.discount_code_id" class="error">
				{{ localFormCopy.errors.discount_code_id }}
			</div>
		</div>
		...
		<div class="col-span-12 space-y-2 md:col-span-4">
			...
			<SimpleSelect
				v-model="localFormCopy.special_offer_id"
				:disabled="localFormCopy.club_reservation"
				:options="specialOfferOptions"
				class="w-full" />
			<div v-if="localFormCopy.errors && localFormCopy.errors.special_offer_id" class="error">
				{{ localFormCopy.errors.special_offer_id }}
			</div>
		</div>
	</div>
	...
	<div
		...
		<TextInput
			v-model="localFormCopy.price"
			:disabled="localFormCopy.club_reservation"
			@input="priceChangedManually" />
		<div v-if="localFormCopy.errors && localFormCopy.errors.price" class="error">
			{{ localFormCopy.errors.price }}
		</div>
	</div>
	...
	<div
		...
		<TextContainer class="max-w-full">
			{{ formatAmount(localFormCopy.club_reservation ? 0 : finalPrice) }}
		</TextContainer>
	</div>
	<div
		class="col-span-12 space-y-2 md:col-span-4"
	...
	<div
		v-if="clubSettings.bulbStatus && !clubSettings['timersStatus'] && !!slotFeatures.slot_has_bulb.length && dayjs().diff(localFormCopy.start_at, 'minutes') > -30"
...
import { ref } from 'vue';
import { Game, SelectOption, Slot } from '@/Types/models';
