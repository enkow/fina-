			...
			<div class="hidden w-full grid-cols-6 gap-x-3 md:grid 2xl:hidden">
				...
				<div class="col-span-2">
					<GameFilter
						...
						class="!w-full"
						custom-route="club.games.statistics.main"
					...
					<StatisticContainer>
						...
						<div class="flex h-full flex-col justify-center">
							...
							<h2 class="-mt-1 mb-0 text-lg font-semibold 2xl:text-xl">
								...
								{{
									formatAmount(
										(paymentType !== 2 ? statistics.averageReservationsTurnover.online : 0) +
											(paymentType !== 1 ? statistics.averageReservationsTurnover.offline : 0),
									)
								}}
							</h2>
							<div v-if="paymentType === 3">
					...
					<StatisticContainer v-if="!(fixedReservationDurationValue === 24)">
						...
						<div class="flex h-full flex-col justify-center">
							...
							<h2 class="-mt-1 mb-0 text-lg font-semibold 2xl:text-xl">
								...
								{{
									formatHoursAndMinutes(
										(paymentType !== 2 ? statistics.averageReservationsHours.online : 0) +
											(paymentType !== 1 ? statistics.averageReservationsHours.offline : 0),
										2,
									)
								}}
							</h2>
					...
					<StatisticContainer>
						...
						<div class="flex h-full flex-col justify-center">
							...
							<h2 class="-mt-1 mb-0 text-lg font-semibold 2xl:text-xl">
								...
								{{
									formattedFloat(
										(paymentType !== 2 ? statistics.averageReservationsCount.online : 0) +
											(paymentType !== 1 ? statistics.averageReservationsCount.offline : 0),
										2,
									).replace('.', ',')
								}}
							</h2>
...
import { DiscountCode, Game, SelectOption, SpecialOffer } from '@/Types/models';
...
const exportType = useExport('club.games.statistics.main.export', {
	game: props.game.id,
});
...
// promotion = mix of special_offers and discount_codes
let promotionOptions = ref<SelectOption[]>([{ code: 'all', label: wTrans('reservation.all-reservations') }]);
const promotion = ref<string>(queryValue(window.location.search, `filters[promotion]`));

// prepare promotion array
props.discountCodes
	.filter((discountCode: DiscountCode) => discountCode.game_id)
	.forEach((discountCode: DiscountCode) => {
		promotionOptions.value.push({
			code: `discountCode_${discountCode.id}`,
			label: wTrans('discount-code.singular_value', {
				value:
					discountCode.code.length <= 11 ? discountCode.code : discountCode.code.substring(0, 10) + '...',
			}),
		});
	});
props.specialOffers
	.filter((specialOffer: SpecialOffer) => specialOffer.game_id)
	.forEach((specialOffer: SpecialOffer) => {
		promotionOptions.value.push({
			code: `specialOffer_${specialOffer.id}`,
			label: wTrans('special-offer.singular-value', {
				value:
					specialOffer.name.length <= 11 ? specialOffer.name : specialOffer.name.substring(0, 10) + '...',
			}),
		});
	});
...
createFilterWatcher<string>('filters[promotion]', promotion);
