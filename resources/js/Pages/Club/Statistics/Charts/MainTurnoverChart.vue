...
const onlineData: number[] = Object.keys(props.statistics.chart.online).map(
	(key) => props.statistics.chart.online[key]['turnover'],
);
const offlineData: number[] = Object.keys(props.statistics.chart.offline).map(
	(key) => props.statistics.chart.offline[key]['turnover'],
);
...
let { options, defaultDatasetValues } = useChart({
	tooltipLabelCallback: function (context): string | string[] {
		...
		if (context.datasetIndex === 0 && props.paymentType === 3) {
			return [
				usePage().props.translations['statistics.tooltip-turnover'] +
					formatAmount(offlineData[context.dataIndex] * 100 + onlineData[context.dataIndex] * 100),
				`${label}`,
			];
		}
		return [`${label}`];
...
if ([1, 3].includes(props.paymentType)) {
	datasets.push(
		Object.assign({}, defaultDatasetValues, {
			label: usePage().props.translations['statistics.tooltip-online-turnover'],
			data: onlineData,
		}),
	);
}
...
if ([2, 3].includes(props.paymentType)) {
	datasets.push(
		Object.assign({}, defaultDatasetValues, {
			label: usePage().props.translations['statistics.tooltip-offline-turnover'],
