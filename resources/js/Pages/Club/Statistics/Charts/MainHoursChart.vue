...
const onlineData: number[] = Object.keys(props.statistics.chart.online).map(
	(key) => props.statistics.chart.online[key]['hours'],
);
const offlineData: number[] = Object.keys(props.statistics.chart.offline).map(
	(key) => props.statistics.chart.offline[key]['hours'],
);
...
if ([1, 3].includes(props.paymentType)) {
	datasets.push(
		Object.assign({}, defaultDatasetValues, {
			label: usePage().props.translations['statistics.tooltip-online-hours'],
			data: onlineData,
		}),
	);
}
...
if ([2, 3].includes(props.paymentType)) {
	datasets.push(
		Object.assign({}, defaultDatasetValues, {
			label: usePage().props.translations['statistics.tooltip-offline-hours'],
