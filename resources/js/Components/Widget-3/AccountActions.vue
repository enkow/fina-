<template>
	<div class="wrapper">
		<Popper
			class="countries"
			:show="countriesShowingDebounced"
			:class="{
				'opacity-0': !(
					countries.length > 1 || !countries.find((country) => country.code === widgetStore.currentCountry)
				),
			}">
			<button
				class="countries__button"
				@click="countriesToggle"
				:class="{
					'cursor-default': !(
						countries.length > 1 || !countries.find((country) => country.code === widgetStore.currentCountry)
					),
				}">
				{{ widgetStore.getCountryLocaleCode(widgetStore.currentCountry) }}
			</button>
			<template #content>
				<ul class="country__list" v-click-away="countriesHide">
					<li
						v-for="country in widgetStore.club.widget_countries"
						class="countries__list_item"
						@click="setCountry(country)">
						{{ widgetStore.getCountryLocaleCode(country) }}
					</li>
				</ul>
			</template>
		</Popper>
...
const countriesShowing = ref<boolean>(false);
const countriesShowingDebounced = refDebounced(countriesShowing, 10);
const countries = widgetStore.club.widget_countries ?? widgetStore.props['countries'];

function countriesHide() {
	countriesShowing.value = false;
}

function countriesToggle() {
	setTimeout(() => {
		if (countriesShowing.value === false) {
			countriesShowing.value = !countriesShowingDebounced.value;
		}
	}, 1);
}
...
function setCountry(country) {
	countriesHide();
	widgetStore.setLocale(country.locale);
	widgetStore.currentCountry = country.code;
}

function logout() {
	axios.post(route('widget.customers.logout', { club: widgetStore.club })).then(() => {
