...
class StoreOpeningHoursExceptionRequest extends FormRequest
{
	/**
	...
	public function rules(): array
	{
		...
		return [
			...
			'reservation_end' => [
				...
				function ($attribute, $value, $fail) {
					if (
						($this->parseTime(request()->get('club_end'))->lt($this->parseTime($value)) &&
							$this->parseTime(request()->get('club_end'))->gt(
								$this->parseTime($this->get('club_start'))
							)) ||
						($this->parseTime(request()->get('club_end'))->gt($this->parseTime($value)) &&
							$this->parseTime(request()->get('club_end'))->lt(
								$this->parseTime($this->get('club_start'))
							))
					) {
						$fail(__('opening-hours.reservation-hours-not-in-opening-hours'));
