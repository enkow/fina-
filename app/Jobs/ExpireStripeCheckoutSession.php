...
class ExpireStripeCheckoutSession implements ShouldQueue
{
		...
		try {
			...
			foreach ($this->payment->payable->reservationSlots as $reservationSlot) {
				if ($reservationSlot->status !== ReservationSlotStatus::Confirmed) {
					...
					if ($numberModel) {
						$numberModel->cancel(true, ReservationSlotCancelationType::System, null);
					}
			...
			$external_account = $this->payment->paymentMethod->external_id;
			$isExternal = $this->payment->paymentMethod->club_id !== null;
